...
终端交互（先做一次“准备”再三步选择）：
  0. 准备：先用当前 config.json 运行 Script/setup_sing_box_service.sh 重启服务
     （需 sudo，默认执行）。否则刚导入/更新过配置时，运行中的实例仍是旧节点，
     Clash API 里的节点名与新配置对不上，无法测速 / 实时切换（列表全显“超时”、
     切换会 404）。可输入 n 跳过。
...
设计目标是尽量兼容不同 config.json：
  ...
  - 列出节点时经 Clash API 并发实时测速，延迟与节点一并显示（服务未运行则跳过）。
...
def api_delay(base: str, headers: dict, name: str) -> "int | None":
    """经 Clash API 实测单个节点/分组延迟，单位 ms；超时/失败返回 None。"""
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=DELAY_TIMEOUT_MS / 1000 + 2) as resp:
            return json.load(resp).get("delay")
    except (urllib.error.URLError, OSError, json.JSONDecodeError, ValueError):
        # HTTPError(测速超时返回非 2xx) 也属于 URLError 子类，统一当作不可用。
        return None


def measure_delays(api: "tuple[str, dict]", names: "list[str]") -> "dict[str, int | None]":
    """并发测速一批节点，返回 {name: ms|None}。"""
    base, headers = api
    if not names:
        return {}
    with ThreadPoolExecutor(max_workers=min(16, len(names))) as ex:
        return dict(zip(names, ex.map(lambda n: api_delay(base, headers, n), names)))


def fmt_delay(ms: "int | None") -> str:
    """给延迟上色：绿<300、黄<800、红其余、灰超时。"""
    if ms is None:
        return "\033[90m超时\033[0m"
    color = "32" if ms < 300 else "33" if ms < 800 else "31"
    return f"\033[{color}m{ms}ms\033[0m"
...
def run_setup() -> int:
    """运行 setup_sing_box_service.sh（需 sudo），用当前 config.json 重启服务。
    返回脚本退出码。"""
...
def main() -> None:
    ...
    # 第 0 步：先用当前 config.json 重启服务，让运行实例与磁盘配置一致。
    # 否则刚导入/更新过配置时，运行实例仍是旧节点，Clash API 里的节点名与新配置
    # 对不上，测速会全显“超时”、实时切换会 404。默认执行，可输入 n 跳过。
    print("\n第 0 步 · 同步运行配置以便测速")
    print("  刚导入/更新过配置的话，运行中的服务还是旧节点，无法准确测速 / 切换；")
    print("  先用当前 config.json 运行 setup_sing_box_service.sh 重启即可对齐。")
    try:
        ans = input("先运行 setup 重启服务以同步配置？[Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        ans = ""
        print()
    if ans in ("", "y", "yes"):
        restart_service()
        time.sleep(1.5)  # 等服务 / Clash API 起来再探测

    # 服务在跑（Clash API 可达）时才能实时测速 / 实时切换；只探测一次。
    api = clash_base(config)
    api_ok = bool(api and api_reachable(*api))

    total = sum(len(v) for v in buckets.values())
    print(f"\n配置文件：{config_path}")
    print(f"目标分组：{group_tag}（{total} 个节点，{len(groups)} 个子分组）")
    print("延迟：已连上 Clash API，列表将实时测速。\n" if api_ok
          else "延迟：Clash API 不可达（服务未运行？），跳过测速。\n")
    ...
    while selected is None:
        ...
        while True:
            ...
            if api_ok:
                print("  测速中…", end="\r", flush=True)
                delays = measure_delays(api, items)
                print(" " * 12, end="\r")  # 清掉“测速中…”
            for i, tag in enumerate(items, 1):
                suffix = f"   {fmt_delay(delays[tag])}" if api_ok else ""
                print(f"  {i}) {tag}{suffix}")
