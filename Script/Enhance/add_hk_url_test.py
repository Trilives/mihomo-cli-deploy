#!/usr/bin/env python3
"""Add a Hong Kong url-test group to a Mihomo config.
...
from add_hk_fallback import collect_hong_kong_proxies
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Create or update a Hong Kong url-test proxy group.",
    )
    parser.add_argument(
        "config",
        nargs="?",
...
def update_config(config_path: Path, group_name: str, url: str, interval: int) -> None:
    ...
    url_test_group = build_url_test_group(group_name, hong_kong_proxies, url, interval)
