...
# it refreshes the mihomo core, Web UI, and geo databases, then reinstalls
# and restarts the mihomo service.

set -euo pipefail

TIMER_NAME="mihomo-update"
...
usage() {
  cat <<'EOF'
Usage:
  sudo ./Script/setup_weekly_update_timer.sh [options]
  sudo ./Script/setup_weekly_update_timer.sh --remove [options]

Options:
  -n, --name <name>        Timer/service base name, default: mihomo-update
...
Examples:
  ...
  sudo ./Script/setup_weekly_update_timer.sh --on-calendar "Mon *-*-* 04:30:00"
  sudo ./Script/setup_weekly_update_timer.sh --remove
EOF
}
...
if [[ ${EUID} -ne 0 ]]; then
  echo "Error: please run as root, for example: sudo ./Script/setup_weekly_update_timer.sh" >&2
  exit 1
fi
...
[Unit]
Description=Weekly mihomo core, UI, and geo database update (${TIMER_NAME})
