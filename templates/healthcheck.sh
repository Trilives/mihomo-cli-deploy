...
# Watchdog probe for the mihomo service.
#
# mihomo can enter a "soft death": the process stays alive but, after the
# uplink interface appears late at boot or drops/changes, auto-detect-interface
...
#   2. If an uplink exists but proxying through the mixed-port fails after a
#      few tries, mihomo is stuck -> restart it.
...
# Underlying tun device created by mihomo; excluded from uplink detection so
# the tun itself is never mistaken for real connectivity.
TUN_DEV="${TUN_DEV:-mihomo}"
...
import json, sys
# mihomo 运行时配置为 JSON 内容（合法 YAML）；读 mixed-port / port。
try:
    data = json.load(open(sys.argv[1], encoding="utf-8"))
except Exception:
    sys.exit(0)
port = data.get("mixed-port") or data.get("port")
if port:
...
# True if a default route exists via something other than the mihomo tun,
