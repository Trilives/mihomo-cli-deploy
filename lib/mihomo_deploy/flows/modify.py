...
def _sub_add() -> None:
    ...
    if info is None:
        ...
        return
    name, url, stype, overlay_flag = info
    set_active = manager.get_active() is None or menu.confirm("设为生效订阅？", default=True)
    manager.add(name, url, stype, apply_overlay=overlay_flag, set_active=set_active)
    if set_active:
        _maybe_node_select()
...
def _service_settings() -> None:
    ...
    if act == 0:
        ...
        shell.run_root(["systemctl", "restart", "mihomo.service"], check=False)
    else:
        service.sync_and_restart()
