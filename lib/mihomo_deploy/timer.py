...
def _service_text() -> str:
    return f"""[Unit]
Description=Weekly Mihomo core, UI, and geo update ({TIMER_NAME})
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart={paths.ROOT / 'mihomo.sh'} update
"""
...
def menu_flow() -> None:
    ...
    if not installed:
        ...
        install(on_calendar=menu.ask("OnCalendar 表达式", default=DEFAULT_ONCALENDAR))
    else:
        remove()
...
def run(argv: list[str] | None = None) -> int:
    import argparse
    p = argparse.ArgumentParser(prog="mihomo_deploy.timer")
