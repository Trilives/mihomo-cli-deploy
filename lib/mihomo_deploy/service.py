"""systemd 服务管理：在 /etc/mihomo 暂存自包含运行时并注册服务。

把内核、配置、geo 数据、UI 暂存到 /etc/mihomo（mihomo 的工作目录 -d），
并把配置内的 external-ui 改写为该目录下的绝对路径，使服务与源码目录
（可能在 /home，有权限/路径耦合问题）解耦。

mihomo 在工作目录根查找 geoip.metadb / geosite.dat；选组持久化（profile.store-selected）
自动写到工作目录 cache.db，无需预建。
...
def _runtime_paths(name: str) -> dict[str, Path]:
    ...
    return {
        ...
        "ui": d / "ui",
        "geoip": d / paths.GEOIP_METADB.name,
        "geosite": d / paths.GEOSITE_DAT.name,
        "unit": Path(f"/etc/systemd/system/{name}.service"),
    }


def _stage_runtime_config(rt: dict[str, Path]) -> Path:
    """读 state/config.yaml（JSON 内容），把 external-ui 改写为运行时绝对路径，写临时文件返回。

    输出仍为 JSON 内容、.yaml 后缀——mihomo 把 JSON 当合法 YAML 解析（见 ARCHITECTURE.md §5）。
    """
    data = json.loads(paths.CONFIG_FILE.read_text("utf-8"))
    if isinstance(data, dict) and data.get("external-ui"):
        data["external-ui"] = str(rt["ui"])
...
def _preflight() -> None:
    if not paths.MIHOMO_BIN.exists():
        raise RuntimeError("未找到 mihomo 内核，请先执行『下载内核/UI/geo 数据』。")
    if not paths.CONFIG_FILE.exists():
        raise RuntimeError("未找到生效配置 state/config.yaml，请先添加订阅。")
    if not (paths.GEOIP_METADB.exists() and paths.GEOSITE_DAT.exists()):
        raise RuntimeError("未找到 geo 数据（geoip.metadb / geosite.dat），请先执行『下载内核/UI/geo 数据』。")
...
def install(name: str = DEFAULT_NAME, *, start: bool = True) -> None:
    """注册并（可选）启动服务。会先移除同名及冲突的 sing-box 服务。"""
    _preflight()
    ...
    try:
        ...
        # geo 数据（mihomo 在工作目录根查找）
        shell.run_root(["install", "-m", "0644", str(paths.GEOIP_METADB), str(rt["geoip"])])
        shell.run_root(["install", "-m", "0644", str(paths.GEOSITE_DAT), str(rt["geosite"])])
        # UI
        ...
        # 运行时校验
        shell.run_root([str(rt["bin"]), "-t", "-d", str(rt["dir"]), "-f", str(rt["config"])], reason="校验配置")
    finally:
...
def sync_and_restart(name: str = DEFAULT_NAME) -> None:
    """把当前 state/config.yaml 同步到运行时并重启服务。"""
...
def remove(name: str = DEFAULT_NAME, *, purge_runtime: bool = True) -> None:
    ...
    if purge_runtime:
        rt = _runtime_paths(name)
        shell.run_root(["rm", "-f", str(rt["config"])], check=False)
        # 若 /etc/mihomo 下不再有其它 <name>.yaml，则整目录删除
        remaining = shell.run_root(
            ["bash", "-c", f"ls {paths.ETC_DIR}/*.yaml 2>/dev/null | wc -l"],
