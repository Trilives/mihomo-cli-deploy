...
移植自参考仓库 setup_resilience.sh + sing_box_healthcheck.sh。解决 mihomo 在网卡
...
healthcheck 探针装到 /etc/mihomo/healthcheck.sh（self-contained）。
"""
...
def install(name: str = "mihomo", *, interval: str = "2min", debounce: int = 20, tun_dev: str = "mihomo") -> None:
...
def run(argv: list[str] | None = None) -> int:
    import argparse
    p = argparse.ArgumentParser(prog="mihomo_deploy.resilience")
    p.add_argument("action", choices=["install", "remove"])
    p.add_argument("-n", "--name", default="mihomo")
